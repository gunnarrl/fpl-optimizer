...
class FPLOptimizer:
    ...
    def setup_paths(self):
        ...
        # Define path for the DGW file
        self.dgw_file = os.path.join(self.data_dir, "processed", 'processed_fpl_data.csv') # Assuming this path
    ...
    def load_data(self):
        """Load and process prediction data and DGW data"""
        ...
        # --- NEW: Load and process DGW data ---
        self.dgw_players_by_gw = {}
        if os.path.exists(self.dgw_file):
            logger.info(f"Loading DGW data from: {self.dgw_file}")
            dgw_df = pd.read_csv(self.dgw_file)
            ...
            if not dgw_df_filtered.empty:
                ...
                if 'element' in dgw_df_filtered.columns and 'GW' in dgw_df_filtered.columns:
                    ...
                    for gw in dgw_df_filtered['GW'].unique():
                        ...
                        players_in_dgw = self.unique_players_df[self.unique_players_df['team'].isin(dgw_teams)]['element'].tolist()
                        self.dgw_players_by_gw[gw] = players_in_dgw
            ...
            logger.warning(f"DGW data file not found: {self.dgw_file}. Chip optimization for DGWs will be limited.")
    ...
    def optimize_single_gw(self, gw, current_squad=None, bank=0, free_transfers=1, use_chips=True, used_chips=None, use_planning_horizon=True):
        ...
        for p in self.players:
            ...
            if 'triple_captain' in chip_vars and (primary_gw, p) in triple_captain_active:
                objective += triple_captain_active[(primary_gw, p)] * pred.get((primary_gw, p), 0) * (self.config['chips']['triple_captain']['captain_multiplier'] - 1)
        ...
        # Add a bonus to the objective if a chip is used during a DGW
        # This incentivizes using the chip during these weeks
        if use_chips:
            for chip_name, chip_info in self.config['chips'].items():
                if chip_name in chip_vars and gw in chip_info['available_gws'] and chip_name not in used_chips:
                    is_dgw_gw = gw in self.dgw_players_by_gw and len(self.dgw_players_by_gw[gw]) > 0

                    if is_dgw_gw:
                        # You'll need to tune these bonus factors based on testing and FPL logic.
                        # For example, a DGW bonus for Bench Boost should reflect the expected points from the bench.
                        # A DGW bonus for Triple Captain should reflect the expected additional points from a DGW captain.
                        dgw_bonus_factor = 0 # Placeholder, adjust this value
                        if chip_name == 'bench_boost':
                            # Example: A bonus of 20 points for using Bench Boost in a DGW
                            # This is a heuristic to encourage DGW usage.
                            dgw_bonus_factor = 50
                        elif chip_name == 'triple_captain':
                            # Example: A bonus of 30 points for using Triple Captain in a DGW
                            dgw_bonus_factor = 50
                        elif chip_name == 'free_hit':
                            # Free Hit utility is complex, often best in BGWs or very large DGWs.
                            # You might adjust this based on the number of DGW players.
                            dgw_bonus_factor = 25

                        if dgw_bonus_factor > 0:
                            objective += chip_vars[chip_name] * dgw_bonus_factor
                            logger.info(f"Added DGW bonus of {dgw_bonus_factor} to objective for {chip_name} in GW{gw}")
    ...
    def optimize_season(self, start_gw=1, end_gw=38):
        ...
        # Pre-identify potential DGWs for chip usage if fixture data is available
        potential_chip_gws = {}
        if hasattr(self, 'dgw_players_by_gw') and self.dgw_players_by_gw:
            for chip_name, chip_info in self.config['chips'].items():
                if chip_name in ['triple_captain', 'bench_boost', 'free_hit']:
                    potential_chip_gws[chip_name] = [
                        gw for gw in chip_info['available_gws']
                        if gw in self.dgw_players_by_gw
                        and len(self.dgw_players_by_gw[gw]) >= 5 # Arbitrary threshold for "significant" DGW
                    ]
                    # Sort by number of DGW players descending, or by aggregate predicted points for DGW players
                    potential_chip_gws[chip_name].sort(key=lambda x: len(self.dgw_players_by_gw.get(x, [])), reverse=True)
                    logger.info(f"Potential {chip_name} GWs: {potential_chip_gws[chip_name]}")
            ...
            try:
                ...
                if solution:
                    ...
                    if solution['chips_used']:
                        ...
                        # Remove used chip from potential_chip_gws for future GWs
                        for used_chip in solution['chips_used']:
                            if used_chip in potential_chip_gws:
                                del potential_chip_gws[used_chip]
...
def main():
    ...
    try:
        ...
        results = optimizer.optimize_season(36, 38)  # Optimize first 6 gameweeks

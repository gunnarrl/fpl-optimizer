...
def process_gameweek_ag(gw, season, df, config):
    ...
    for i in range(1, PLANNING_HORIZON + 1):
        ...
        try:
            ...
            # Define arguments to pass to AutoGluon's .fit() method
            ag_fit_args = {
                'resources': {'num_gpus': 1, 'num_cpus': 8},
                # Tell the internal Ray instances not to start a dashboard to avoid port conflicts
                'ray_init_args': {
            ...
            # Pass the combined arguments dictionary
            predictor.fit(
                train_data,
                presets=PRESET_QUALITY,
                time_limit=TIME_LIMIT_PER_MODEL,
                ag_args_fit=ag_fit_args  # <--- USE THE NEW DICTIONARY
            )
